...
if __name__ == "__main__":
    ...
    mse = skmts.mean_squared_error(pred_df[y_df.name], pred_df['prediction'])
    rmse = np.sqrt(mse)
    r_square = skmts.r2_score(pred_df[y_df.name], pred_df['prediction'])
    pcc, pval = scistat.pearsonr(pred_df[y_df.name], pred_df['prediction'])
